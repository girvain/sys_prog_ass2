...
void send_time(int socket)
{
  time_t t;    // always look up the manual to see the error conditions
    ...
    writen(socket, (unsigned char *) &n, sizeof(size_t));	
    writen(socket, (unsigned char *) asctime(tm), n);	  // Cwk2: server.c - multi-threaded server using readn() and writen()
...
void send_time(int socket)
{
  time_t t;    // always look up the manual to see the error conditions
    ...
    size_t n = strlen(asctime(tm)) + 1;
    writen(socket, (unsigned char *) &n, sizeof(size_t));	
    writen(socket, (unsigned char // Cwk2: server.c - multi-threaded server using readn() and writen()
